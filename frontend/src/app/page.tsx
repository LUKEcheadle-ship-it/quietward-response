    ...
    <div className="space-y-8">
      <section className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div>
          ...
          <h1 className="page-title">Investigate what changed. Preserve why it matters.</h1>
          <p className="muted mt-3 max-w-3xl">Deterministic correlation turns validated sensor events into explainable incidents, timelines, and policy-safe next steps.</p>
        </div>
        <div className="rounded-lg border border-cyan/20 bg-cyan/5 px-4 py-3 text-xs text-cyan">
          Phase 1 · diagnostic guidance only · zero endpoint actions
        </div>
      </section>
          ...
          <section className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
            <div>
              ...
              <div className="space-y-3">
                {data.recent_incidents.length === 0 && <div className="panel text-sm text-slate-400">No incidents yet. Run the safe demo seeder to exercise the pipeline.</div>}
            ...
            <aside className="panel h-fit">
              ...
              <p className="muted mt-3">Recommendations are deterministic and separated into investigation versus remediation. No process, file, service, firewall, or isolation action can execute in Phase 1.</p>
              <div className="mt-5 space-y-3 text-sm">
                ...
                <div className="flex justify-between border-b border-line pb-3"><span className="text-slate-400">Audit trail</span><span className="text-emerald-300">Enabled</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Remediation</span><span className="text-amber-200">Phase 2</span></div>
              </div>
            </aside>
          </section>
        </>
