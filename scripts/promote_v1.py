...
def _frontend_documents() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if lock.get("version") != package.get("version") or root_package.get("version") != package.get("version"):
        raise RuntimeError("frontend package/package-lock versions are inconsistent before promotion")
...
def _promote_frontend_versions() -> None:
    package, lock, root_package = _frontend_documents()
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Validate every deterministic promotion marker without modifying files.",
    )
    ...
    if args.check:
        ...
        print("All RC markers and frontend package/lock metadata are promotion-ready.")
        return 0
    ...
    _replace_once(
        CHANGELOG,
        CHANGELOG_PROMOTION_TEXT,
        'After promotion, rerun the complete release wrapper and the documented UI smoke '
        'check before merge, tag, or public-release publication.',
    )
    ...
    print(
        "Next: review these deterministic version-only changes, commit them, then rerun "
        "scripts/finalize_v1.py before merge/tag/publication."
    )
    return 0
